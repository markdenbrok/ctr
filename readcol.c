...
int readcol(char *file,char *flags,char *fmt, ...) {
  ...
  totargs=nargs_readcol(fmt);
  /* This exercise is just to make sure we don't cross array boundaries 
     when we do the strncmp below... */
  ...
  while(fgets(buffer,bufsize,fp)!=NULL){
    if (buffer[0]!=skipsym && buffer[0]!='\n'){
      ...
      va_end(ap);
      va_start(ap,fmt);
      //if (vsscanf(buffer,fmt,ap)==totargs){
...
void readcol_add_to_end(int ntotal,int filelen,char *fmt,va_list ap){
  /*Programming this in c is diff because parameters are protected for editing
    Instead of changing the pointers in the parameter list,
...
void sort_stuff_readcol(int filelen,char *fmt,va_list ap){
  /* Since all read values are stored from the back of the array to the front,
     we have to reverse them again. */
  const char *fp;
  char **s, **st,*sq;
  char *c, *ct,cq;
  int *i, *it,iq;
